...
set max_j [get_property BUS_WIDTH [get_nets inst_StaticPlaceholder/NET_X1Y5_east[0]]]
...
set data [get_nodes -of_objects  [get_nets inst_StaticPlaceholder/NET_X1Y5_east[$j]]]
foreach line $data {
...
set max_j [get_property BUS_WIDTH [get_nets inst_StaticPlaceholder/NET_X1Y5_west[0]]]
...
set data [get_nodes -of_objects  [get_nets inst_StaticPlaceholder/NET_X1Y5_west[$j]]]
foreach line $data {
if [regexp  "(INT.*)+(/)+(.*)" $line match5 tile slash wire] {
  puts $fileId "ExcludePortsFromBlocking PortName=$wire Location=$tile CheckForExistence=False IncludeAllPorts=True;";  
}
