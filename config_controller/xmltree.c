...
void
parseClkFreq(xmlDocPtr doc, xmlNode * a_node)
{
    xmlNode *cur_node = NULL;
    xmlChar *key;
    ...
    for (cur_node = a_node; cur_node; cur_node = cur_node->next) {
        if (cur_node->type == XML_ELEMENT_NODE) {
            if (!xmlStrcmp(cur_node->name, (const xmlChar *)"clk_freq")) {
                key = xmlNodeListGetString(doc, cur_node->children, 1);
                clk_freq = atoi(key);
                xmlFree(key);
            }
        ...
        parseClkFreq(doc, cur_node->children);
    }
}

void
parseWGVector(xmlDocPtr doc, xmlNode * a_node)
{
    xmlNode *cur_node = NULL;
    xmlChar *key;
    ...
    for (cur_node = a_node; cur_node; cur_node = cur_node->next) {
        if (cur_node->type == XML_ELEMENT_NODE) {
            if (!xmlStrcmp(cur_node->name, (const xmlChar *)"wg_vector")) {
                key = xmlNodeListGetString(doc, cur_node->children, 1);
                wg_vector = atoi(key);
                xmlFree(key);
            }
        ...
        parseWGVector(doc, cur_node->children);
    }
}

void
parseInputParamWidth(xmlDocPtr doc, xmlNode * a_node)
{
    xmlNode *cur_node = NULL;
    xmlChar *key;
    ...
    for (cur_node = a_node; cur_node; cur_node = cur_node->next) {
        if (cur_node->type == XML_ELEMENT_NODE) {
            if (!xmlStrcmp(cur_node->name, (const xmlChar *)"input_param_width")) {
                key = xmlNodeListGetString(doc, cur_node->children, 1);
                input_param_width = atoi(key);
                xmlFree(key);
            }
        }

        parseInputParamWidth(doc, cur_node->children);
    }
}

void
parseOutputParamWidth(xmlDocPtr doc, xmlNode * a_node)
{
    xmlNode *cur_node = NULL;
    xmlChar *key;
    ...
    for (cur_node = a_node; cur_node; cur_node = cur_node->next) {
        if (cur_node->type == XML_ELEMENT_NODE) {
            if (!xmlStrcmp(cur_node->name, (const xmlChar *)"output_param_width")) {
                key = xmlNodeListGetString(doc, cur_node->children, 1);
                output_param_width = atoi(key);
                xmlFree(key);
            }
        }

        parseOutputParamWidth(doc, cur_node->children);
    }
}
/**
